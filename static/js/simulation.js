...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnRetry) btnRetry.addEventListener("click", startSimulation);

    async function startSimulation() {
        showScreen(activeScreen);
        questionArea.innerHTML = '<div class="loading">Caricamento esercizi...</div>';

        try {
            const res = await fetch("/api/simulation/exercises");
            if (!res.ok) throw new Error("Errore nel caricamento degli esercizi");
    ...
    function updateNavigator() {
        ...
        buttons.forEach((btn, i) => {
            ...
            if (state.answered && state.correct === true) {
                btn.classList.add("sim-nav-answered");
            } else if (state.answered && state.correct === false) {
                btn.classList.add("sim-nav-answered");
    ...
    function renderCurrentQuestion() {
        ...
        if (ex.steps) {
            ...
            ex.steps.forEach((step, i) => {
                ...
                html += `<div class="step${selectedClass}${lockedClass}" data-index="${i}">Passo ${i + 1}: ${step}</div>`;
            });
            html += "</div>";
        }
        ...
        if (ex.options) {
            ...
            ex.options.forEach((opt, i) => {
                ...
                const selectedClass = isSelected ? " selected" : "";
                const lockedClass = state.answered ? " locked" : "";
                html += `<div class="option${selectedClass}${lockedClass}" data-index="${i}">${opt}</div>`;
            });
            html += "</div>";
        }
        ...
        if (!state.answered) {
            const selectables = ex.steps
                ? questionArea.querySelectorAll(".step")
                : questionArea.querySelectorAll(".option");
    ...
    btnAnswer.addEventListener("click", async () => {
        ...
        try {
            ...
            // Lock clicks on options/steps
            questionArea.querySelectorAll(".option, .step").forEach(el => {
    ...
    function calculateResults() {
        ...
        document.getElementById("sim-results-subtitle").textContent =
            `Tempo impiegato: ${formatTime(TOTAL_TIME - remainingTime)} su 50:00`;
    ...
    function generateSuggestions(byType, totalScore, avgTime) {
        ...
        if (totalSkipped > 5) {
            suggestions.push(
                ...
                `per sentirti piu sicuro durante la simulazione.`
            );
        }
        ...
        if (avgTime > 180) {
            suggestions.push(
                ...
                `Prova a gestire meglio il tempo nelle prossime simulazioni.`
            );
        }
        ...
        if (totalScore >= 16) {
            ...
            suggestions.push("C'e ancora lavoro da fare. Pratica ogni tipo di esercizio separatamente prima di ripetere la simulazione.");
        } else {
            suggestions.push(
